...
class GeometriesTest {
    ...
    void testFindIntersections() {
        ...
        assertNull(geometries.findIntersections(new Ray(new Point(0,1,0),new Vector(1,0,0))),
                "empty collection");
        ...
        assertEquals(1,
                geometries.findIntersections(new Ray(new Point(-3, -4, -1), new Vector(4, -3, 1)))
                        .size());

        // intersects with all the shapes
        assertEquals(4,
                geometries.findIntersections(new Ray(new Point(0, 0.5, 6), new Vector(0, 0, -12)))
                        .size());

        // ============ Equivalence Partitions Tests ==============
        //checks for expected more than one shape intersecting
        assertEquals(3,
                geometries.findIntersections(new Ray(new Point(0, 3, 6), new Vector(0, 0, -12)))
                        .size());
    }
}